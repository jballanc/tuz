...
(defn away-from
  ...
  (let [diff-x (- x center-x)
        ...
        dist-y (game/single-dimension-distance y center-y
                                               (count game-map))]
    (condp = 0
      dist-x (rand-nth [:east :west])
      dist-y (rand-nth [:north :south])
      (match [(< dist-x dist-y)
              (= dist-y (Math/abs diff-y))
              (pos? diff-y)
              (= dist-x (Math/abs diff-x))
              (pos? diff-x)]
            [true true true _ _] :south
            [true true false _ _] :north
            [true false true _ _] :north
            [true false false _ _] :south
            [false _ _ true true] :east
            [false _ _ true false] :west
            [false _ _ false true] :west
            [false _ _ false false] :east))))
